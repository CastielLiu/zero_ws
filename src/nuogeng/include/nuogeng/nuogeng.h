...
#define MAX_NOUT_SIZE 105
#define MAX_PKG_LEN 106
...
private:
	void BufferIncomingData(unsigned char *message,size_t len);
	void parseMessage();
	verify_width_t generateCRC(unsigned char *buf,unsigned int nBytes);
	void crcTableGenerate(unsigned char *crc_table);
	uint8_t generateLRC(uint8_t* buf,size_t len);
...
private:
	...
	size_t data_index_;
	NuogengGps_t *gpsPtr;
	gps_msgs::Inspvax inspax;
};




static verify_width_t crcTable[256]={
